 ...
 *************************************************
 * Modified for readability and conformity to angular standards.
 * By: Dustin Ratcliffe
 * Date: 05-17-2017
 *************************************************
 */
...
/* App Module */
var UploadUI = app || angular.module('UploadUI', []);
UploadUI.value('fileUploaderOptions', {
	url: '/',
	alias: 'file',
	headers: {},
	queue: [],
...
UploadUI.directive('ngFileSelect', ['$parse', 'FileUploader', 'FileSelect', function ($parse, FileUploader, FileSelect){
	return {
...
UploadUI.directive('ngFileDrop', ['$parse', 'FileUploader', 'FileDrop', function ($parse, FileUploader, FileDrop){
	return {
		link: function link(scope, element, attributes){
			var uploader = scope.$eval(attributes.uploader);
...
UploadUI.directive('ngFileOver', ['FileUploader', 'FileOver', function (FileUploader, FileOver){
	return {
		link: function link(scope, element, attributes){
			var uploader = scope.$eval(attributes.uploader);
...
UploadUI.factory('FileUploader', ['fileUploaderOptions', '$rootScope', '$http', '$window', '$timeout', 'FileLikeObject', 'FileItem', 'Pipeline',
		function (fileUploaderOptions, $rootScope, $http, $window, $timeout, FileLikeObject, FileItem, Pipeline){
	var FileUploader = (function (){
		...
		FileUploader.prototype.addToQueue = function addToQueue(files, options, filters){
			...
			var next = function next(){
				...
				if (angular.isUndefined(something)){
					return done();
				}
		...
		FileUploader.prototype.destroy = function destroy(){
			...
			angular.forEach(this._directives, function (key){
				angular.forEach(_this3._directives[key], function (object){
					object.destroy();
				});
			});
		}
		...
		FileUploader.prototype._convertFiltersToPipes = function _convertFiltersToPipes(filters){
			...
			return filters.map(function (filter){
				var fn = angular.bind(_this4, filter.fn);
		...
		FileUploader.prototype._iframeTransport = function _iframeTransport(item){
			...
			angular.forEach(item.formData, function (obj){
				angular.forEach(obj, function (value, key){
					var element_ = angular.element('<input type="hidden" name="' + key + '" />');
					element_.val(value);
...
UploadUI.factory('FileLikeObject', [function (){
...
UploadUI.factory('FileItem', ['$compile', 'FileLikeObject', function ($compile, FileLikeObject){
...
UploadUI.factory('FileDirective', [function (){
...
UploadUI.factory('FileSelect', ['FileDirective', function (FileDirective){
...
UploadUI.factory('Pipeline', ['$q', function ($q){
	var Pipeline = (function (){
		...
		Pipeline.prototype.next = function next(args){
			...
			if (pipe.isAsync){
				...
				var onFulfilled = angular.bind(this, this.next, args);
				var onRejected = angular.bind(this, this.onThrown, err);
...
UploadUI.factory('FileDrop', ['FileDirective', function (FileDirective){
...
UploadUI.factory('FileOver', ['FileDirective', function (FileDirective){
